...
class BidirectionalSelfAttention(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb=None, input_indices=None):
...
class Block(nn.Module):
    ...
    def forward(self, x, rotary_pos_emb=None, input_indices=None):
        x = x + self.attn(self.ln_1(x), rotary_pos_emb, input_indices)
...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rotary_embedding = RotaryEmbedding(32)
    ...
    def forward(self, ids, id_indices=None):
        device = ids.device
        b, t = ids.size()
        ...
        for block in self.transformer.h:
            x = block(x, self.rotary_embedding, id_indices)
...
class Predictor(nn.Module):
    ...
    def forward(self, embeddings, id_indices=None):
        device = embeddings.device
        assert embeddings.shape[-1] == self.config.n_embd, f"bad number of dimensions in input embedding {embeddings.shape}, expected {self.config.n_embd}"
        # b, t = embeddings.size()
        # assert t <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
        # pos = torch.arange(0, t, dtype=torch.long, device=device) # shape (t)
