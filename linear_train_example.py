...
b = 3
target_block_num = 2
tokenized['input_ids'] = torch.arange(0, b * ins).view(b, ins)
tokenized['attention_mask'] = torch.ones((b, ins), dtype = torch.bool)
tokenized['attention_mask'][0, -1:] = False
...
# PREDICTIONS WITH CONTEXT MASKING AND TARGET MASKING
...
# we are repeating the index permutation in case high - block_size is smaller than target_block_num, this will lead to duplicate targets TODO: maybe should be masked out
...
for index, target_block_range in enumerate(target_block_indices.view(batch_count, -1)):
    allowed_in_context[index,target_block_range] = False # set the indices of the target blocks to false. If the target block size is smaller than the max target block size, then False will be written to the non updated indices multiple times, but this is ok since they are already False
...
#%%
# zero out the predicted target embeddings which were masked
...
#%%
predicted_target_embeddings.shape
...
target_block_ranges = torch.cat([torch.randperm(high  - block_size)[:target_block_num] for high in true_input_lengths])# select target block ranges for each target block
...
predicted_target_embeddings = predicted_embeddings[:,context_length:] # remove the context predictions
# compute the loss of the masked predictions
loss = F.smooth_l1_loss(predicted_target_embeddings, target_blocks_embeddings.view(predicted_target_embeddings.shape)) # compute the loss
