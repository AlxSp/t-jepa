...
def compute_ce_loss(
    ...
    # add ones along the diagonal  of each so each input attends to at least one other input
    decoder_attn_mask = prediction_attn_mask + torch.eye(predicted_target_embeddings.shape[1]).repeat(batch_count, 1, 1)

    assert not torch.equal(decoder_attn_mask, prediction_attn_mask)

    out = decoder(predicted_target_embeddings, prediction_input_indices[:,context_length:].to(device), attn_mask=decoder_attn_mask.unsqueeze(1).to(device))
    ...
    relevant_target_attn_mask = relevant_target_attn_mask.reshape(-1).to(device)
