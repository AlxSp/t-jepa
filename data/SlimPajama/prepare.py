...
dataset = load_dataset("cerebras/SlimPajama-627B", cache_dir=dataset_dir, num_proc=num_proc)
#%%
# tokenized = dataset.map(tokenize_and_cast, num_proc=num_proc)
#%%
dataset = dataset.filter(lambda x: tokenizer(x['text'], return_tensors="pt", add_special_tokens=False, return_attention_mask=False).shape[1] >= 64, num_proc=num_proc)
